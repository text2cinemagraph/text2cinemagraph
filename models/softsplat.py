#!/usr/bin/env python
# Code adapted from:
# https://github.com/gangsterless/pytorch-softmax-splatting
...
kernel_Maximumsplat_updateOutput = '''
	extern "C" __global__ void kernel_Maximumsplat_updateOutput(
		const int n,
		const float* input,
		const float* flow,
		float* output
		...
		if ((intNorthwestX >= 0) & (intNorthwestX < SIZE_3(output)) & (intNorthwestY >= 0) & (intNorthwestY < SIZE_2(output))) {
            float val = VALUE_4(input, intN, intC, intY, intX) * fltNorthwest;
            int* address_as_i = (int*)(&output[OFFSET_4(output, intN, intC, intNorthwestY, intNorthwestX)]);
		...
		if ((intNortheastX >= 0) & (intNortheastX < SIZE_3(output)) & (intNortheastY >= 0) & (intNortheastY < SIZE_2(output))) {
            float val = VALUE_4(input, intN, intC, intY, intX) * fltNortheast;
            int* address_as_i = (int*)(&output[OFFSET_4(output, intN, intC, intNortheastY, intNortheastX)]);
            int old = *address_as_i, assumed;
            do {
                            assumed = old;
                            old = atomicCAS(address_as_i, assumed,
		...
		if ((intSouthwestX >= 0) & (intSouthwestX < SIZE_3(output)) & (intSouthwestY >= 0) & (intSouthwestY < SIZE_2(output))) {
            float val = VALUE_4(input, intN, intC, intY, intX) * fltSouthwest;
            int* address_as_i = (int*)(&output[OFFSET_4(output, intN, intC, intSouthwestY, intSouthwestX)]);
            int old = *address_as_i, assumed;
            do {
                            assumed = old;
                            old = atomicCAS(address_as_i, assumed,
		...
		if ((intSoutheastX >= 0) & (intSoutheastX < SIZE_3(output)) & (intSoutheastY >= 0) & (intSoutheastY < SIZE_2(output))) {
            float val = VALUE_4(input, intN, intC, intY, intX) * fltSoutheast;
            int* address_as_i = (int*)(&output[OFFSET_4(output, intN, intC, intSoutheastY, intSoutheastX)]);
            int old = *address_as_i, assumed;
            do {
                            assumed = old;
                            old = atomicCAS(address_as_i, assumed,
...
kernel_Inversesplat_updateOutput = '''
	extern "C" __global__ void kernel_Inversesplat_updateOutput(
		const int n,
		const float* max_warped_input,
		const float* flow,
		float* output
	) { for (int intIndex = (blockIdx.x * blockDim.x) + threadIdx.x; intIndex < n; intIndex += blockDim.x * gridDim.x) {
		const int intN = ( intIndex / SIZE_3(output) / SIZE_2(output) / SIZE_1(output) ) % SIZE_0(output);
		...
		int* address_as_i = (int*)(&output[OFFSET_4(output, intN, intC, intY, intX)]);
		if ((intNorthwestX >= 0) & (intNorthwestX < SIZE_3(output)) & (intNorthwestY >= 0) & (intNorthwestY < SIZE_2(output))) {
            float val = VALUE_4(max_warped_input, intN, intC, intNorthwestY, intNorthwestX);
		...
		if ((intNortheastX >= 0) & (intNortheastX < SIZE_3(output)) & (intNortheastY >= 0) & (intNortheastY < SIZE_2(output))) {
            float val = VALUE_4(max_warped_input, intN, intC, intNortheastY, intNortheastX);
		...
		if ((intSouthwestX >= 0) & (intSouthwestX < SIZE_3(output)) & (intSouthwestY >= 0) & (intSouthwestY < SIZE_2(output))) {
            float val = VALUE_4(max_warped_input, intN, intC, intSouthwestY, intSouthwestX);
		...
		if ((intSoutheastX >= 0) & (intSoutheastX < SIZE_3(output)) & (intSoutheastY >= 0) & (intSoutheastY < SIZE_2(output))) {
            float val = VALUE_4(max_warped_input, intN, intC, intSoutheastY, intSoutheastX);
...
class _FunctionMaximumsplat(torch.autograd.Function):
	@staticmethod
	def forward(self, input, flow):
		self.save_for_backward(input, flow)
		...
		if input.is_cuda == True:
			n = output.nelement()
			cupy_launch('kernel_Maximumsplat_updateOutput', cupy_kernel('kernel_Maximumsplat_updateOutput', {
				'input': input,
				'flow': flow,
				'output': output
			}))(
...
"""
class _FunctionMaximumWarpNormsplat(torch.autograd.Function):
	@staticmethod
	def forward(self, input, flow):
		self.save_for_backward(input, flow)
		...
		max_warped_input = input.new_zeros([ intSamples, intInputDepth, intInputHeight, intInputWidth ])

		if input.is_cuda == True:
			n = max_warped_input.nelement()
			cupy_launch('kernel_Maximumsplat_updateOutput', cupy_kernel('kernel_Maximumsplat_updateOutput', {
				'input': input,
				'flow': flow,
				'output': max_warped_input
			}))(
				grid=tuple([ int((n + 512 - 1) / 512), 1, 1 ]),
				block=tuple([ 512, 1, 1 ]),
				args=[ n, input.data_ptr(), flow.data_ptr(), max_warped_input.data_ptr() ]
			)

		elif input.is_cuda == False:
			raise NotImplementedError()

		output = input.new_zeros([ intSamples, intInputDepth, intInputHeight, intInputWidth ])#input.clone()
		if input.is_cuda == True:
			n = output.nelement()
			cupy_launch('kernel_Inversesplat_updateOutput', cupy_kernel('kernel_Inversesplat_updateOutput', {
				'max_warped_input': max_warped_input,
				'flow': flow,
				'output': output
			}))(
				grid=tuple([int((n + 512 - 1) / 512), 1, 1]),
				block=tuple([512, 1, 1]),
				args=[n, max_warped_input.data_ptr(), flow.data_ptr(), output.data_ptr()]
			)
...
def _FunctionMaximumWarpNormsplat(input, flow):
	#self.save_for_backward(input, flow)
	...
	max_warped_input = input.new_zeros([ intSamples, intInputDepth, intInputHeight, intInputWidth ]) - 1000.0
	...
	output = input.clone()#input.new_zeros([ intSamples, intInputDepth, intInputHeight, intInputWidth ])
	if input.is_cuda == True:
		n = output.nelement()
		cupy_launch('kernel_Inversesplat_updateOutput', cupy_kernel('kernel_Inversesplat_updateOutput', {
			'max_warped_input': max_warped_input,
...
class _FunctionInversesplat(torch.autograd.Function):
	@staticmethod
	def forward(self, input, flow):
		self.save_for_backward(input, flow)
...
class ModuleMaximumsplat(torch.nn.Module):
	def __init__(self):
		super(ModuleMaximumsplat, self).__init__()
	# end
	def forward(self, tenInput, tenFlow):
		return _FunctionMaximumsplat.apply(tenInput, tenFlow)
	# end
# end



class ModuleMaximumWarpNormsplat(torch.nn.Module):
	def __init__(self):
		super(ModuleMaximumWarpNormsplat, self).__init__()
	# end
	def forward(self, tenInput, tenFlow):
		#return _FunctionMaximumWarpNormsplat.apply(tenInput, tenFlow)
		return _FunctionMaximumWarpNormsplat(tenInput, tenFlow)
	# end
# end