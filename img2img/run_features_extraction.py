...
from cldm.util import instantiate_from_config
...
def twin_extraction(config, model=None, sampler=None):
    ...
    if exp_config.twin_extraction.init_img != '':
        exp_config.twin_extraction.seed = -1
        exp_config.twin_extraction.prompt = ""
        exp_config.twin_extraction.scale = 1.0

    print('prompt: ', exp_config.twin_extraction.prompt)
        
    seed = exp_config.twin_extraction.seed 
    ...
    save_feature_timesteps = exp_config.twin_extraction.ddim_steps if exp_config.twin_extraction.init_img == '' else exp_config.twin_extraction.save_feature_timesteps

    outpath = f"{exp_path_root}/{exp_config.twin_extraction.experiment_name}"
    ...
    with open(os.path.join(outpath, "args.json"), "w") as f:
        args_to_save = OmegaConf.to_container(exp_config.twin_extraction)
    ...
    def ddim_sampler_callback(pred_x0, xt, i, outpath=None):
    ...
    def save_feature_maps(blocks, i, feature_type="input_block"):
        ...
        for block in tqdm(blocks, desc="Saving input blocks feature maps"):
            if not exp_config.twin_extraction.save_all_features and block_idx < 4:
                block_idx += 1
                continue
            if "ResBlock" in str(type(block[0])):
                if exp_config.twin_extraction.save_all_features or block_idx == 4:
    ...
    def save_feature_maps_callback(i):
        if exp_config.twin_extraction.save_all_features:
    ...
    assert exp_config.twin_extraction.prompt is not None
    prompts = [exp_config.twin_extraction.prompt]
    ...
    with torch.no_grad():
        with precision_scope("cuda"):
            with model.ema_scope():
                ...
                shape = [exp_config.twin_extraction.C, exp_config.twin_extraction.H // exp_config.twin_extraction.f, 
                            exp_config.twin_extraction.W // exp_config.twin_extraction.f]

                z_enc = None
                if exp_config.twin_extraction.init_img != '':
                    assert os.path.isfile(exp_config.twin_extraction.init_img)
                    init_image = load_img(exp_config.twin_extraction.init_img).to(device)
                    init_latent = model.get_first_stage_encoding(model.encode_first_stage(init_image))
                    ddim_inversion_steps = 999
                    z_enc, _ = sampler.encode_ddim(init_latent, num_steps=ddim_inversion_steps, conditioning=c,unconditional_conditioning=uc,unconditional_guidance_scale=exp_config.twin_extraction.scale)
                else:
                    z_enc = torch.randn([1, exp_config.twin_extraction.C, exp_config.twin_extraction.H // exp_config.twin_extraction.f, 
                                        exp_config.twin_extraction.W // exp_config.twin_extraction.f], device=device)
                torch.save(z_enc, f"{outpath}/z_enc.pt")
                samples_ddim, _ = sampler.sample(S=exp_config.twin_extraction.ddim_steps,
                                conditioning=c,
                                batch_size=1,
                                shape=shape,
                                verbose=False,
                                unconditional_guidance_scale=exp_config.twin_extraction.scale,
...
def main():
    ...
    print('prompt: ', exp_config.config.prompt)
