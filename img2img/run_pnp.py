...
from cldm.models.diffusion.ddim import DDIMSampler
from run_features_extraction import load_model_from_config

def twin_generation(config, model=None, sampler=None):
    exp_config = config
    exp_path_root = exp_config.twin_generation.exp_path_root
    
    # read seed from args.json of source experiment
    with open(os.path.join(exp_path_root, exp_config.twin_extraction.experiment_name, "args.json"), "r") as f:
        args = json.load(f)
        seed = args["seed"]
        source_prompt = args["prompt"]
    negative_prompt = source_prompt if exp_config.twin_generation.negative_prompt is None else exp_config.twin_generation.negative_prompt

    seed_everything(seed)
    possible_ddim_steps = args["save_feature_timesteps"]
    assert exp_config.twin_generation.num_ddim_sampling_steps in possible_ddim_steps or exp_config.twin_generation.num_ddim_sampling_steps is None, f"possible sampling steps for this experiment are: {possible_ddim_steps}; for {exp_config.twin_generation.num_ddim_sampling_steps} steps, run 'run_features_extraction.py' with save_feature_timesteps = {exp_config.twin_generation.num_ddim_sampling_steps}"
    ddim_steps = exp_config.twin_generation.num_ddim_sampling_steps if exp_config.twin_generation.num_ddim_sampling_steps is not None else possible_ddim_steps[-1]
    ...
    if model is None:
        model = load_model_from_config(model_config, f"{exp_config.twin_extraction.ckpt}")
        model = model.to(device)
    
    if sampler is None:
        sampler = DDIMSampler(model)
        
    sampler.make_schedule(ddim_num_steps=ddim_steps, ddim_eta=exp_config.twin_extraction.ddim_eta, verbose=False) 

    seed = torch.initial_seed()
    exp_config.twin_extraction.seed = seed

    outpaths = [os.path.join(f"{exp_path_root}/{exp_config.twin_extraction.experiment_name}/translations")]
    ...
    def load_target_features():
        ...
        feature_injection_thresholds = [exp_config.twin_generation.feature_injection_threshold]
        target_features = []

        source_experiment_out_layers_path = os.path.join(exp_path_root, exp_config.twin_extraction.experiment_name, "feature_maps")
        source_experiment_qkv_path = os.path.join(exp_path_root, exp_config.twin_extraction.experiment_name, "feature_maps")
    ...
    batch_size = 1
    prompts = [exp_config.twin_generation.prompt]
    assert prompts is not None

    start_code_path = f"{exp_path_root}/{exp_config.twin_extraction.experiment_name}/z_enc.pt"
    ...
    precision_scope = autocast if exp_config.twin_extraction.precision=="autocast" else nullcontext
    ...
    with torch.no_grad():
        with precision_scope("cuda"):
            with model.ema_scope():
                uc = None
                nc = None
                if exp_config.twin_generation.scale != 1.0:
                ...
                shape = [exp_config.twin_extraction.C, exp_config.twin_extraction.H // exp_config.twin_extraction.f, exp_config.twin_extraction.W // exp_config.twin_extraction.f]
                samples_ddim, _ = sampler.sample(S=ddim_steps,
                                                 ...
                                                 shape=shape,
                                                 verbose=False,
                                                 unconditional_guidance_scale=exp_config.twin_generation.scale,
                                                 unconditional_conditioning=uc,
                                                 eta=exp_config.twin_extraction.ddim_eta,
                                                 x_T=start_code,
                                                #  img_callback=ddim_sampler_callback,
                                                 img_callback=None,
                                                 injected_features=injected_features,
                                                 negative_prompt_alpha=exp_config.twin_generation.negative_prompt_alpha,
                                                 negative_prompt_schedule=exp_config.twin_generation.negative_prompt_schedule,
                                                 )
                ...
                for k, x_sample in enumerate(x_image_torch):
                    x_sample = 255. * rearrange(x_sample.cpu().numpy(), 'c h w -> h w c')
                    img = Image.fromarray(x_sample.astype(np.uint8))
                    img.save(os.path.join(exp_path_root, exp_config.twin_extraction.experiment_name, 'translations/sample_0.png'))
                    sample_idx += 1

    print(f"PnP results saved in: {os.path.join(exp_path_root, exp_config.twin_extraction.experiment_name, 'translations/sample_0.png')}")
...
def main():
    ...
    parser.add_argument('--ckpt', type=str, default='../dreambooth/models/ldm/stable-diffusion-v1/sd-v1-4-full-ema.ckpt', help='model checkpoint')
        ...
        with precision_scope("cuda"):
            with model.ema_scope():
                ...
                for k, x_sample in enumerate(x_image_torch):
                    x_sample = 255. * rearrange(x_sample.cpu().numpy(), 'c h w -> h w c')
                    img = Image.fromarray(x_sample.astype(np.uint8))
                    img.save(os.path.join(exp_path_root, exp_config.source_experiment_name, 'translations/sample_0.png'))
                    sample_idx += 1
