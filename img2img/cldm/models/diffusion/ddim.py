...
from cldm.modules.diffusionmodules.util import make_ddim_sampling_parameters, make_ddim_timesteps, noise_like, \
    extract_into_tensor
...
class DDIMSampler(object):
    ...
    def ddim_sampling(self, cond, shape, negative_conditioning=None,
        ...
        print(f"Running DDIM ANIRUDDHA Sampling with {total_steps} timesteps")
    ...
    def p_sample_ddim(self, x, c, t, index, negative_conditioning=None,
        ...
        if negative_conditioning is not None:
            ...
            if isinstance(nc, dict):
                ...
                c_in = dict()
                for k in nc:
                    if isinstance(nc[k], list):
                        c_in[k] = [torch.cat([
                            nc[k][i],
                            uc[k][i]]) for i in range(len(nc[k]))]
                    else:
                        c_in[k] = torch.cat([
                                nc[k],
                                uc[k]])
                ...
                for i in range(len(nc)):
                    c_in.append(torch.cat([nc[i], uc[i]]))
            else:
