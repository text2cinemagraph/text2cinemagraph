    ...
    for i, data in enumerate(dataset, start=epoch_iter):
        ...
        if opt.use_prompts:
            losses, generated = model(Variable(data['label']), Variable(data['inst']), 
                Variable(data['image']), Variable(data['feat']), Variable(data['prompt']), infer=save_fake)
        else:
            losses, generated = model(Variable(data['label']), Variable(data['inst']), 
        ...
        loss_G = loss_dict['G_GAN'] + loss_dict.get('G_GAN_Feat',0) + loss_dict.get('G_EPE', 0) + loss_dict.get('G_L1', 0)
        ...
        if save_fake:
            ...
            if data['label'].shape[1] == 4:
                visuals['mask'] = util.tensor2im(data['label'][0][3:4,:,:])
            if 'hint' in data.keys():
                visuals['hint_flow'] = flow2img(data['hint'][0])
