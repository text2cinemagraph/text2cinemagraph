...
if __name__ == "__main__":
    ...
    parser.add_argument('--idx', type=int, help='index of cpu')
    ...
    with ExitStack() as stack:
        ...
        if args.input:
            files = glob.glob(args.input+'/*_input.jpg')
            files.sort()
            ...
            for i in tqdm.tqdm(range(len(files))):
                ...
                if args.output:
                    #saving mask
                    name = os.path.basename(path).replace('_input.jpg', '') + '_mask.png'
                    out_filename = os.path.join(args.output, name)
