...
def load_demo(args):
    cfg = LazyConfig.load(args.odise.config_file)
    ...
    if args.odise.vocab:
        for words in args.odise.vocab.split(";"):
    ...
    ODISECheckpointer(model).load(args.odise.init_from)
...
def odise_mask(args, inference_model=None, demo_metadata=None, aug=None):

    if inference_model is None or demo_metadata is None or aug is None:
        cfg = LazyConfig.load(args.odise.config_file)
        cfg.model.overlap_threshold = 0
        cfg.model.clip_head.alpha = 0.35
        cfg.model.clip_head.beta = 0.65
    ...
    with ExitStack() as stack:
        ...
        if args.odise.input:
            if len(args.odise.input) == 1:
                args.odise.input = glob.glob(os.path.expanduser(args.odise.input[0]))
                assert args.odise.input, "The input path(s) was not found"
            for path in tqdm.tqdm(args.odise.input, disable=not args.odise.output):
                # use PIL, to be consistent with evaluation
                img = utils.read_image(path, format="RGB")
                start_time = time.time()
                predictions, visualized_output = demo.run_on_image(img)

                if args.odise.output:
                    if os.path.isdir(args.odise.output):
                        assert os.path.isdir(args.odise.output), args.odise.output
                        out_filename = os.path.join(args.odise.output, os.path.basename(path))
                    else:
                        assert len(args.odise.input) == 1, "Please specify a directory with args.output"
                        out_filename = args.odise.output
                    # visualized_output.save(out_filename)
                    mask = ((predictions['panoptic_seg'][0].cpu().numpy() != 0.) * 255.).astype(np.uint8)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config-file",
        default="ODISE/configs/Panoptic/odise_label_coco_50e.py",
        type=str,
